...
1. Ensuite, vous appliquez une `Time Interval` à la première mesure. Cliquez sur **[!UICONTROL Time Options]** (icône d’horloge) à droite de la `first Revenue metric`.
...
1. Dans la liste déroulante, définissez les options suivantes :
   ...
   * Ensuite, vous ajoutez votre `Day number of the month` dimension en cliquant sur **[!UICONTROL Group By]** et en sélectionnant la dimension. Cela renverra le numéro du jour du mois d’une commande ; par exemple, une commande passée le 2 mars renverra `2`.

   * Dans le `Group By` menu déroulant, sélectionnez `Show All` et cliquez sur **[!UICONTROL Apply]**. Les valeurs de l’axe X sont ainsi créées pour le rapport :
