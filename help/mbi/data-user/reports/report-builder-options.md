...
Examinez quelques-unes des raisons les plus courantes pour lesquelles vous utiliseriez la variable [!DNL SQL Report Builder] via la [!DNL traditional Report Builder].
...
Au mieux, vous devez patienter un cycle de mise à jour avant de pouvoir utiliser votre colonne. Si vous réalisez que vous avez commis une erreur lors de la création de la colonne, vous devez attendre *two* cycles : un pour remplir initialement la colonne et un autre cycle pour propager les révisions.
...
Supposons qu’un seul produit soit considéré comme faisant partie de plusieurs catégories de produits et que vous souhaitiez afficher les recettes associées à chaque catégorie de chaque produit. Tenter de créer cette relation à l’aide de la gestion des actifs numériques (DWM) peut être fastidieux et difficile, mais écrire une [!DNL SQL] requête peut être un peu plus simple :
