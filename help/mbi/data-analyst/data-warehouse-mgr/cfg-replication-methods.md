...
>[!NOTE]
>
>Contrairement au `Modified At` méthode, `Add Date` ne vérifie pas les lignes existantes à la recherche d’informations mises à jour ; elle se tourne uniquement vers les nouvelles lignes.
...
Dans l&#39;un ou l&#39;autre des scénarios ci-dessus, `Full Table` la réplication ne détecte aucune modification et vos données deviennent obsolètes. En raison de l’inefficacité de cette méthode de réplication, et des exigences mentionnées ci-dessus, `Full Table` la réplication n’est recommandée qu’en dernier recours.

### Lot de clé Principal

Lorsqu’un tableau utilise `Primary Key Batch` (Lot PK), de nouvelles données sont découvertes en comptant les lignes à l’intérieur de plages, ou lots, de valeurs de clé Principales. Bien que vous pensiez généralement que cela est utilisé avec des entiers, même les valeurs de texte peuvent être triées de manière à permettre au système de définir des plages constantes.
