...
* Rejoindre à `customer_group` tableau pour créer des colonnes qui renvoient le nom du groupe de clients du client qui a passé la commande.
   * Chemin : `sales_order.customer_group_id` (nombreux) => `customer_group.customer_group_id` (1)

`sales_order_address`

* Rejoindre à `sales_order_address` tableau pour créer des colonnes qui renvoient les emplacements de facturation et d’expédition associés à la commande. Deux chemins de jointure sont possibles, selon que les détails de facturation ou de livraison sont requis.
   * Chemins :
...
* Rejoindre à `store` pour créer des colonnes qui renvoient des détails relatifs à la boutique Commerce associée à la commande.
