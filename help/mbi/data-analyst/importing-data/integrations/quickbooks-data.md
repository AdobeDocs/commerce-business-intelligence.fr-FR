...
|-----|-----|
| [`bill`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Bill) | Le `bills` contient des informations sur les transactions AP ou une demande de paiement auprès d’un tiers. Les attributs incluent le type de devise, le taux de change, le montant total, la date d’échéance, le solde, etc. |
...
| [`creditmemos`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/CreditMemo) | Le `creditmemos` le tableau enregistre les transactions qui sont des remboursements ou des crédits de paiements complets et partiels. Certains attributs incluent le nom du client, les informations de facturation et d’expédition du client, le montant et la date. |
...
| [`journalentries`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/JournalEntry) | Le `journalentries` le tableau enregistre les informations de compte AR et AP, y compris l’identifiant de saisie du journal, la date de transaction et les informations sur l’article. |
...
| [`purchases`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Purchase) | Le `purchases` représente vos dépenses et inclut l’identifiant d’achat, le type de paiement, le montant et les informations sur les articles. |
| [`purchaseorders`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/PurchaseOrder) | Le `purchaseorders` table contient les demandes de produits envoyées aux fournisseurs. Ce tableau comprend les informations sur le fournisseur, l’identifiant de commande d’achat, la date de transaction, les informations sur les éléments de ligne, le montant total et les informations sur le compte AP. |
| [`refundreceipts`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/RefundReceipt) | Le `refundreceipts` consigne les remboursements aux clients. Les attributs incluent l’identifiant de reçu du remboursement, les informations sur l’article, le montant total, les informations sur les clients et le solde. |
| [`salesreceipts`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/SalesReceipt) | Le `salesreceipts` le tableau enregistre des informations dans les reçus de vente remis aux clients, y compris l’identifiant du ticket de caisse, les informations sur l’article, le montant total, les informations sur le client, la date de transaction et tout dépôt. |
| [`timeactivities`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/TimeActivity) | Le `timeactivities` La table contient des enregistrements d’heure pour les fournisseurs et/ou les employés et inclut l’ID d’activité temporelle, les informations sur les employés/fournisseurs, l’heure consignée, la description de l’activité et la date enregistrée. |
| [`transfers`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Transfer) | Le `transfers` le tableau enregistre les informations sur les fonds transférés entre les comptes. Les attributs incluent l’ID de transfert, le montant, les informations de compte et la date. |
...
| [`customers`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Customer) | Le `customers` Le tableau contient toutes les informations relatives à vos clients, notamment l’identifiant, le nom, les adresses de facturation et de livraison, le numéro de téléphone et l’adresse électronique du client. |
| [`departments`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Department) | Le `departments` le tableau comprend l’ID, le nom et le type de service (sous-service ou service de niveau supérieur). |
| [`employees`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Employee) | Le `employees` le tableau enregistre des informations sur les personnes qui travaillent pour votre entreprise. Les attributs incluent l’ID d’employé, le nom, l’adresse, le numéro de téléphone et les informations facturables, si la société est activée sur les salaires. |
| [`items`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/Item) | Le `items` contient des détails sur les produits ou services que votre entreprise vend. Ce tableau comprend l’identifiant de l’article, le nom, la description, le prix unitaire, le type, les informations d’achat, la quantité en main et les informations sur le compte de revenu et de ressource. |
| [`paymentmethods`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/PaymentMethod) | Le `paymentmethods` Le tableau indique le mode de paiement reçu pour les biens et services. Il contient l’ID, le type et le nom du paiement. |
...
| [`taxcodes`](https://developer.intuit.com/app/developer/qbo/docs/api/accounting/all-entities/TaxCode) | Les codes d’imposition servent à effectuer le suivi de la situation fiscale (imposable ou non) des produits, des services et des clients. Le `taxcodes` le tableau comprend l’ID du code fiscal, le nom, la description, le statut, le statut imposable, le taux d’imposition et le groupe fiscal. |
