...
Si vous préférez automatiser la récupération des données de l’une de vos propres sources, vous pouvez utiliser le `Import API` [!DNL Commerce Intelligence] . En gros, s&#39;il n&#39;est pas dans une base de données ou une intégration `SaaS`, la fonction `Import API` est votre meilleur pari.
...
* [Intégrations `SaaS` et `database` disponibles](../integrations/integrations.md)
